...
Structure of the model (DO - dropout):
INPUT - DO -  CONVOLUTION - DO - LSTM (- DO - LSTM) - DO - ATTENTION - DENSE - SOFTMAX
...
Parameters:
    ...
    REC_NUM_UNITS - a list of numbers of units in each LSTM layer (bottom first)
    DENSE_NUM_UNITS - number of units in dense layer
    ATTENTION_NUM_UNITS - number of units in attention layer
    DROPOUT_FRACTION - probability of setting to zero
    ...
    PRINT_PROGRESS - print training and evaluation progress (a dot every 10%) for each dataset
    ...
    PARAMS_TO_LOAD - path to a file with parameters to load (use if you want to continue a finished training)
...
parser.add_argument('-REC_NUM_UNITS', nargs='+', type=int, default=[169, 169])
parser.add_argument('-DENSE_NUM_UNITS', type=int, default=169)
parser.add_argument('-ATTENTION_NUM_UNITS', type=int, default=169)
...
folder_name += '_semeval_block_%d_LSTM' % len(args.REC_NUM_UNITS)
for rnu in args.REC_NUM_UNITS:
    folder_name += '_%d' % rnu
folder_name += '_D%d' % args.DENSE_NUM_UNITS
...
logger.info('TOL                : ' + str(args.TOL))
logger.info('REC_NUM_UNITS      : ' + str(args.REC_NUM_UNITS))
logger.info('DENSE_NUM_UNITS    : ' + str(args.DENSE_NUM_UNITS))
logger.info('ATTENTION_NUM_UNITS: ' + str(args.ATTENTION_NUM_UNITS))
logger.info('DROPOUT_FRACTION   : ' + str(args.DROPOUT_FRACTION))
logger.info('LEARNING_RATE      : ' + str(args.LEARNING_RATE))
logger.info('DECAY              : ' + str(args.DECAY))
logger.info('NO_DECAY_EPOCHS    : ' + str(args.NO_DECAY_EPOCHS))
logger.info('MAX_GRAD           : ' + str(args.MAX_GRAD))
logger.info('NUM_EPOCHS         : ' + str(args.NUM_EPOCHS))
...
logger.info('PRINT_PROGRESS     : ' + str(args.PRINT_PROGRESS))
logger.info('DATA_PATH          : ' + str(args.DATA_PATH))
logger.info('CREATE_DATA_CHUNKS : ' + str(args.CREATE_DATA_CHUNKS))
logger.info('PARAMS_TO_LOAD     : ' + str(args.PARAMS_TO_LOAD))
logger.info('DROPOUT_TYPE       : ' + str(args.DROPOUT_TYPE))
logger.info('TRAIN_IDS          : ' + str(args.TRAIN_IDS))
logger.info('VALID_ID           : ' + str(args.VALID_ID))
logger.info('TEST_ID            : ' + str(args.TEST_ID))
logger.info('EVAL_IDS           : ' + str(args.EVAL_IDS))
...
logger.info('Loading data took {:.2f}s'.format(time.time()-t0))
...
logger.info('charset_map     : ' + str(data.charset_map))
...
# block control layer
l_control = lasagne.layers.LSTMLayer(incoming=l_inp, mask_input=l_mask, num_units=100,
                                     ...
                                     learn_init=True, precompute_input=True, grad_clipping=args.MAX_GRAD)
...
if args.CONV_SIZES is not None:
    ...
    if args.DROPOUT_TYPE == 'word':
        l_drp = word_dropout.WordDropoutLayer(incoming=l_conv, word_input=l_inp, space=data.charset_map[' '],
                                              p=args.DROPOUT_FRACTION)
    else:
        l_drp = lasagne.layers.DropoutLayer(incoming=l_conv, p=args.DROPOUT_FRACTION)

# Block layer
l_block = block.BlockLayer(incoming=l_drp, control_incoming=l_control, num_units=100, mask_input=l_mask)
...
for i_layer, rec_num_units in enumerate(args.REC_NUM_UNITS):
    ...
    # LSTM
    l_lstm = lasagne.layers.LSTMLayer(incoming=l_lstm, mask_input=l_mask, num_units=rec_num_units,
                                      ingate=lasagne.layers.Gate(W_in=INI, W_hid=INI, W_cell=INI),
                                      forgetgate=lasagne.layers.Gate(W_in=INI, W_hid=INI, W_cell=INI),
                                      outgate=lasagne.layers.Gate(W_in=INI, W_hid=INI, W_cell=INI),
                                      cell=lasagne.layers.Gate(W_in=INI, W_hid=INI, W_cell=None,
    ...
    # Block layer
    if i_layer < len(args.REC_NUM_UNITS) - 1:
        l_lstm = block.BlockLayer(incoming=l_lstm, control_incoming=l_control, num_units=100, mask_input=l_mask)
...
if args.DENSE_NUM_UNITS > 0:
    l_lstm = lasagne.layers.DenseLayer(incoming=l_lstm, num_units=args.DENSE_NUM_UNITS)
...
logger.info('#NETWORK params: {}'.format(total_params))
...
logger.info('Building the model took {:.2f}s'.format(time.time()-t0))
...
for epoch in range(args.NUM_EPOCHS):
    ...
    evaluation.evaluate()
    measurements = evaluation.get_all_measurements()
    for i_data, cm in zip(evaluation.eval_ids, measurements['CM'][-1]):
        logger.info('Confusion matrix {} - '.format(i_data) + data.dataset_names[i_data])
        logger.info(evaluation.conf_matrix_to_str(cm))

    logger.info('Crossentropy: ' + '/'.join(['{:.3f}'.format(x) for x in measurements['ce'][-1]]))
    logger.info('Accuracy    : ' + '/'.join(['{:.3f}'.format(x) for x in measurements['acc'][-1]]))
    logger.info('F^PN_1      : ' + '/'.join(['{:.3f}'.format(x) for x in measurements['F'][-1]]))
    logger.info('Recall      : ' + '/'.join(['{:.3f}'.format(x) for x in measurements['R'][-1]]))

    elapsed_eval = time.time() - batch_time
    logger.info('Time elapsed : {:.0f} + {:.0f} = {:.0f}s ~= {:.1f}min'.format(elapsed_train, elapsed_eval,
                                                                               elapsed_train + elapsed_eval,
                                                                               (elapsed_train + elapsed_eval) / 60.))
    eta = (elapsed_train + elapsed_eval) * (args.NUM_EPOCHS - epoch - 1) / 60.
    logger.info('ETA          : {:.0f}min ~= {:.1f}h'.format(eta, eta / 60.))

    logger.info('Best valid R = {:.3f} with test R = {:.3f} in epoch {:d}.'.format(evaluation.best_R[1],
                                                                                   evaluation.best_R[2],
                                                                                   evaluation.best_R[0]))

    # Store cross entropy, correct rates and F scores
    sio.savemat(os.path.join(folder_name, 'performance.mat'), measurements)
    ...
    if epoch - evaluation.best_R[0] == args.EARLY_STOPPING:
        logger.info('Validation set recall did not increase in last {:d} epochs. Stopping.'.format(args.EARLY_STOPPING))
        break
